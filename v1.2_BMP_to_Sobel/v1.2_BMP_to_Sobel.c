...
void findEdges(unsigned char* input, unsigned char* output, int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            // 엣지 강도 계산
            int edge = (int)(sqrt((double)(gx * gx + gy * gy)));
...
int main() {
    // 1. 파일명 수정
    const char* inputFile = "galaxy.bmp";
    const char* outputFile = "output_galaxy_grayscale.bmp";
    const char* edgeFile = "output_galaxy_edge.bmp";     // 엣지 검출 BMP 파일 추가
    const char* memFile = "output_galaxy_image.mem";

    const int IMAGE_WIDTH = 12882;
    const int IMAGE_HEIGHT = 12882;
    ...
    // 4. 엣지 검출 BMP 파일 저장 (그레이스케일 저장 후 추가)
